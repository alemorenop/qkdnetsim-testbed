...
int main (int argc, char *argv[])
{
    ...
    std::string tapName3   = "tap-kms3";
    std::string tapName4   = "tap-kms4";
    std::string tapHost3   = "10.250.0.1";   // host-side IP (na Linux TAP-u)
    std::string tapNs3_3   = "10.250.0.2";   // ns-3 IP na n3 (TAP interfejs)
    std::string tapHost4   = "10.250.1.1";   // host-side IP (na Linux TAP-u)
    std::string tapNs3_4   = "10.250.1.2";   // ns-3 IP na n4 (TAP interfejs)
    std::string tapMask    = "255.255.255.0";

    double  appHoldTime = 0.5;
    uint16_t simulationTime = 50000;
    uint16_t appStartTime = 50;
    uint16_t appStopTime = 50000;
    uint16_t qkdStartTime = 0;
    uint16_t qkdStopTime = 50000;
    ...
    emuDevice.SetDeviceName ("ens160");
    ...
    std::string macStringTempNode3 ("00:00:00:00:00:91");
    ...
    Ipv4InterfaceAddress address_emu_node3 = Ipv4InterfaceAddress (Ipv4Address("100.100.129.101"), Ipv4Mask("255.255.0.0"));
    ...
    std::string macStringTempNode4 ("00:00:00:00:00:92");
    ...
    Ipv4InterfaceAddress address_emu_node4 = Ipv4InterfaceAddress (Ipv4Address("100.100.129.102"), Ipv4Mask("255.255.0.0"));
    ...
    TapFdNetDeviceHelper tap3;
    tap3.SetDeviceName(tapName3);
    tap3.SetModePi(false); // bez PI headera
    tap3.SetTapIpv4Address(Ipv4Address(tapHost3.c_str())); // host-side IP
    tap3.SetTapIpv4Mask(Ipv4Mask(tapMask.c_str()));
    NetDeviceContainer tapDev3 = tap3.Install(n.Get(3));
    Ptr<NetDevice> devTap3 = tapDev3.Get(0);
    uint32_t ifTap3 = ipv4_node3->AddInterface(devTap3);
    ipv4_node3->AddAddress(ifTap3, Ipv4InterfaceAddress(Ipv4Address(tapNs3_3.c_str()), Ipv4Mask(tapMask.c_str())));
    ipv4_node3->SetMetric(ifTap3, 1);
    ipv4_node3->SetUp(ifTap3);
 
    TapFdNetDeviceHelper tap4;
    tap4.SetDeviceName(tapName4);
    tap4.SetModePi(false);
    tap4.SetTapIpv4Address(Ipv4Address(tapHost4.c_str())); // host-side IP
    tap4.SetTapIpv4Mask(Ipv4Mask(tapMask.c_str()));
    NetDeviceContainer tapDev4 = tap4.Install(n.Get(4));
    Ptr<NetDevice> devTap4 = tapDev4.Get(0);
    uint32_t ifTap4 = ipv4_node4->AddInterface(devTap4);
    ipv4_node4->AddAddress(ifTap4, Ipv4InterfaceAddress(Ipv4Address(tapNs3_4.c_str()), Ipv4Mask(tapMask.c_str())));
    ipv4_node4->SetMetric(ifTap4, 1);
    ipv4_node4->SetUp(ifTap4);
    ...
    */
    ...
    if(trace){
        ...
        p2p.EnableAsciiAll (ascii.CreateFileStream ("qkd_etis014_emulation_tap.tr"));
        p2p.EnablePcapAll ("qkd_etis014_emulation_tap");
        AnimationInterface anim ("qkd_etis014_emulation_tap.xml");  // where "animation.xml" is any arbitrary filename
    }
