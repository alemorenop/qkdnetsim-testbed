...
3. **VPN integration.** The Python consumer installs each synchronized
   QKD-derived value as a strongSwan IKE pre-shared key. It therefore
   authenticates establishment of a fresh IKE SA; strongSwan derives the ESP
   traffic keys through IKEv2. The current implementation neither injects QKD
   material directly as an ESP key nor implements RFC 8784 PPK. The endpoint
   coordination API is currently plain HTTP inside the emulated classical
   network; protecting that channel with HTTPS is future work.
...
QKD/KMS and CORE infrastructure, runs the four supported VPN variants and
cleans up their containers.
...
- **[`docker/vpn/`](docker/vpn/)** — the strongSwan endpoint image, its
  ...
  buffers before installing the PSK.
