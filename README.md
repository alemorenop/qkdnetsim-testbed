...
2. **Internal QKDNetSim relay.** `Relay()`, `skey_create` and the ETSI 004
   relay-control operations connect several simulated KMS nodes. They are
   implementation-specific KMS-to-KMS mechanisms, not multi-hop APIs defined
   by ETSI GS QKD 004 or ETSI GS QKD 014. The application-facing requests
   remain ETSI-shaped at Alice and Bob.
...
All keys in this repository are generated by QKDNetSim's model. Terms such as
“QKD-derived” and “QKD-backed” describe their role in the emulation, not keys
obtained from physical QKD hardware.
