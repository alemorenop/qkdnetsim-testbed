...
| PPK-backed VPN | RFC 8784 PPK mixing in IKEv2 | Both QKD topologies and both ETSI interfaces supply a mandatory, generation-specific PPK to strongSwan; the authentication PSK is independent |
...
3. **VPN integration.** In the original four variants, the Python consumer
   ...
   The endpoint coordination API is still plain HTTP inside the emulated
   classical network; protecting that channel with HTTPS is future work.
...
reported QKD fingerprint intact, so IKEv2 itself must reject the exchange.
...
after consumption; accounts ETSI 014 `number` and per-key sizes consistently;
and makes duplicate or delayed `fill` acknowledgements idempotent. ETSI 004
...
their inclusive ranges before reconstructing a mixed key. These are runtime
and correctness adaptations around QKDNetSim's imported concatenation policy,
...
applied inside QKDNetSim. Once the value is installed as a PSK, IKEv2 applies
strongSwan's normal PRF-based key schedule to derive the IKE and ESP SAs. This
...
| RFC 8784 PPK VPN | 5/5 | All four ETSI/topology combinations accepted two PPK generations; a mismatched PPK was rejected |
...
The RFC 8784 extension was validated on 21 September 2026 over point-to-point
and trusted-node topologies with both ETSI 004 and ETSI 014. Every positive
case established two fresh IKE SAs reporting mandatory PPK use, retired the
previous SA and sustained one TCP flow through ESP with no clear application
payload. Throughput ranged from 88.53 to 88.94 Mbit/s. The negative case kept
the advertised QKD reference and fingerprint unchanged but installed a
different PPK at Bob; IKEv2 rejected the exchange as required.
The four original PSK combinations and their mismatched-key rejection were
then rerun against the same rebuilt images and also completed successfully.
...
PP/KMS dependencies; and `core/vpn-topology.py` then waits for actual QKD
data-plane evidence before it creates either VPN endpoint. Point-to-point
...
roughly one to two minutes on a cold start. This is expected: do not manually
restart the containers during that interval, because doing so discards the
connection state and restarts convergence. The CORE runner waits for semantic
key/relay markers automatically, so that delay does not consume the VPN
consumer's ETSI retry budget.
...
`ipsec.conf`/`ipsec.secrets` interface; it checks the established SA's PPK
indicator as well as the installed ESP CHILD_SA.
...
and is distinct from the authentication PSK. strongSwan's
[PPK configuration](https://docs.strongswan.org/docs/5.9/swanctl/swanctlConf.html)
provides the corresponding `secrets.ppk` entry.
...
and Bob obtain the same QKD value through their local KMS interfaces and
compare its index and fingerprint before loading it as generation `qkd-N`'s
PPK. The Python consumer loads the connection and secret through VICI, then
establishes a **fresh IKE SA** for each generation. RFC 8784 applies the PPK
...
The negative case leaves the QKD index and reported fingerprint matching but
changes Bob's *installed PPK*. An IKE SA must then fail to establish. This
...
exercises the common strongSwan/IKEv2 stage after key delivery.
The Alice--Bob coordination API remains unauthenticated HTTP, so this is an
emulated integration demonstration and **not** a complete secure key
provisioning design; authenticating that channel remains future work.
...
operator explicitly selects hybrid delivery. Both QKD-only and forced-hybrid
VPN variants have been validated. The relay, ETSI 004 association and original
...
- **[`docker/vpn/`](docker/vpn/)** — the strongSwan endpoint image, its
  ...
  buffers before installing the authentication PSK or a mandatory RFC 8784
  PPK. The PPK mode uses an independent bootstrap authentication secret.
